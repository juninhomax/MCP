Write-Host "🚀 Installation automatique MCP Brain/Slaves - Windows" -ForegroundColor Cyan
...
if (-not $isAdmin) {
    Write-Host "⚠️  Certaines opérations nécessitent des privilèges administrateur" -ForegroundColor Yellow
...
if (Test-Command python) {
    ...
    if ([version]$version -lt [version]"3.11") {
        Write-Host "   ⚠️  Python $version détecté. Python 3.11+ recommandé" -ForegroundColor Yellow
...
# 2. Vérifier Ollama
Write-Host ""
Write-Host "2️⃣ Vérification d'Ollama..." -ForegroundColor Yellow
try {
    ...
    if ($hasQwen) {
        Write-Host "   ✅ Modèle Qwen détecté: $($hasQwen.name)" -ForegroundColor Green
    } else {
        Write-Host "   ⚠️  Modèle Qwen non trouvé" -ForegroundColor Yellow
        Write-Host "   📥 Téléchargement du modèle..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "3️⃣ Configuration de MCP Brain..." -ForegroundColor Yellow
Set-Location brain
...
if (-not (Test-Path ".env")) {
    Write-Host "   ⚙️  Création du fichier de configuration..." -ForegroundColor Gray
    Copy-Item .env.example .env
}

Write-Host "   ✅ MCP Brain configuré" -ForegroundColor Green
deactivate
Set-Location ..

# 4. Configurer l'environnement virtuel pour Windows Slave
Write-Host ""
Write-Host "4️⃣ Configuration de MCP Slave Windows..." -ForegroundColor Yellow
...
Write-Host "   ✅ MCP Slave Windows configuré" -ForegroundColor Green
deactivate
Set-Location ..\..

# 5. Vérifier la politique d'exécution
Write-Host ""
Write-Host "5️⃣ Vérification de la politique d'exécution PowerShell..." -ForegroundColor Yellow
$policy = Get-ExecutionPolicy -Scope CurrentUser
if ($policy -eq "Restricted" -or $policy -eq "Undefined") {
    Write-Host "   ⚠️  Politique d'exécution restrictive détectée: $policy" -ForegroundColor Yellow
    ...
    Write-Host "   ✅ Politique d'exécution OK: $policy" -ForegroundColor Green
}
